...
struct AsyncWorker : std::enable_shared_from_this<AsyncWorker> {
  ...
  void waitUntilIdle() {
    ...
    condition.wait(lock, [this]() {
      return tasks.empty() && !runningTask;
    });
  }
...
struct DatabaseHostObject::DatabaseState {
  ...
  void enqueueAsyncTask(std::function<void()> task) {
    asyncWorker->enqueue(std::move(task));
  }

  void waitUntilAsyncIdle() {
    asyncWorker->waitUntilIdle();
  }

  SqlResultData executeSqlDirect(const std::string& sql, const std::vector<SqlParam>& params) {
    std::lock_guard<std::mutex> lock(mutex);
    ...
    const auto isReadonly = sqlite3_stmt_readonly(stmt.get()) != 0;
    ...
    resultData.rowsAffected = isReadonly ? 0 : sqlite3_changes(db);
  ...
  SqlResultData executeSql(const std::string& sql, const std::vector<SqlParam>& params) {
    waitUntilAsyncIdle();
    return executeSqlDirect(sql, params);
  }
 ...
 public:
  ...
  std::shared_ptr<AsyncWorker> asyncWorker;
  std::mutex mutex;
};
  ...
  if (propName == "executeSqlAsync") {
    ...
    return jsi::Function::createFromHostFunction(runtime, propNameId, 2,
      [weakState](jsi::Runtime& runtime, const jsi::Value&, const jsi::Value* arguments, size_t count) -> jsi::Value {
        ...
        try {
          sql = arguments[0].getString(runtime).utf8(runtime);
          params = parseParams(runtime, arguments[1].getObject(runtime).getArray(runtime));
        ...
        try {
          sharedState->enqueueAsyncTask(
            [sharedState, promise = std::move(promise), sql = std::move(sql), params = std::move(params)]() mutable {
              try {
                promise.resolve(sharedState->executeSqlDirect(sql, params));
              } catch (const std::exception& error) {
                promise.reject(Error(error.what()));
              }
            }
